...
=============================================================
MEJORAS v7.0:
  1. DataClass DocumentoAnalizado — modelo de datos fuerte
  2. Caché JSON ligera — re-análisis sin releer archivos
  3. Hash global del expediente — huella del conjunto
  4. Filtros, búsqueda y ordenamiento en tabla
  5. Historial de ediciones manuales con usuario y timestamp
  6. config_indice.json — institución y tipos configurables
  7. requirements.txt automático

MEJORAS v7.1:
  1. Módulo de Auditoría de Integridad
     - Carga el Excel del índice generado previamente
     - Recalcula SHA-256 de cada archivo en la carpeta Foliados
     - Compara: ÍNTEGRO / MODIFICADO / FALTANTE / DESCONOCIDO
     - Exporta reporte de auditoría en Excel y log .txt
     - Botón "🔍 Auditar" integrado en la interfaz principal
=============================================================
...
APP_TITULO     = "GESTOR DE EXPEDIENTES DIGITALES"
APP_VERSION    = "v7.1"
...
class DocumentoAnalizado:
    ...
    # ── Compatibilidad con acceso tipo diccionario ────────────────────────────
...
def fecha_desde_nombre(nombre: str) -> tuple:
    # Patrón 1: YYYY-MM-DD_HHMM (formato original)
    ...
    # Patrón 2: NNN_YYYY-MM-DD_HHMM (formato foliado con guiones)
    m2=re.match(r'^\d+_(\d{4})-(\d{2})-(\d{2})_(\d{2})(\d{2})',nombre)
    if m2:
        y,mo,d,h,mi=m2.groups()
        try:
            datetime.strptime(f"{y}{mo}{d}","%Y%m%d")
            hora = f"{h}:{mi}" if h != "00" or mi != "00" else ""
            return f"{y}-{mo}-{d}",hora,"Nombre-archivo",f'"{m2.group(0)}"'
        except ValueError: pass
    # Patrón 3: NNN_YYYYMMDD_HHMM (formato foliado sin guiones)
    m3=re.match(r'^\d+_(\d{4})(\d{2})(\d{2})_(\d{2})(\d{2})',nombre)
    if m3:
        y,mo,d,h,mi=m3.groups()
        try:
            datetime.strptime(f"{y}{mo}{d}","%Y%m%d")
            hora = f"{h}:{mi}" if h != "00" or mi != "00" else ""
            return f"{y}-{mo}-{d}",hora,"Nombre-archivo",f'"{m3.group(0)}"'
        except ValueError: pass
    return "","","",""
...
# ══════════════════════════════════════════════════════════════════════════════
# FIRMA DE LÍNEA BASE — expediente_firmado.json
# Permite guardar el estado oficial del expediente en un momento dado
# y verificarlo posteriormente para detectar cualquier alteración.
# ══════════════════════════════════════════════════════════════════════════════

FIRMA_FILENAME = "expediente_firmado.json"


def guardar_firma_base(docs: list, carpeta: str,
                        config: dict = None) -> tuple:
    """
    Guarda la firma de línea base del expediente en expediente_firmado.json.
    Este archivo es la referencia oficial para auditorías futuras.

    Contiene:
      - timestamp de firma
      - institución
      - hash global del conjunto
      - hash individual por archivo con folio y nombre foliado
      - total de documentos y rango de folios

    Retorna (ruta_str, error_str).
    """
    try:
        cfg = config or {}
        ts  = datetime.now().isoformat()

        # Construir hash global
        hash_global = generar_hash_expediente(docs)

        # Construir registro por documento
        documentos = []
        for d in sorted(docs, key=lambda x: getattr(x, "folio", "")):
            documentos.append({
                "folio":          getattr(d, "folio",          ""),
                "nombre":         getattr(d, "nombre",         ""),
                "nombre_foliado": getattr(d, "nombre_foliado", ""),
                "hash_sha256":    getattr(d, "hash_sha256",    ""),
                "tipo":           getattr(d, "tipo",           ""),
                "fecha":          getattr(d, "fecha",          ""),
                "tamano":         getattr(d, "tamano",         ""),
            })

        folios = [getattr(d, "folio", "") for d in docs if getattr(d, "folio", "")]
        fi     = min(folios) if folios else ""
        ff     = max(folios) if folios else ""

        firma = {
            "_descripcion": (
                "Firma de línea base del expediente digital. "
                "Generada por " + APP_VERSION + ". "
                "No modificar manualmente — cualquier cambio invalida la firma."
            ),
            ...
            "seccion":            cfg.get("seccion", ""),
            "titulo":             cfg.get("titulo", ""),
            "folio_inicial":      fi,
            "folio_final":        ff,
            "total_documentos":   len(docs),
            "hash_global":        hash_global,
            "hash_global_corto":  hash_global[:16] + "..." + hash_global[-8:],
            "documentos":         documentos,
        }

        ruta = Path(carpeta) / FIRMA_FILENAME
        with open(ruta, "w", encoding="utf-8") as f:
            json.dump(firma, f, ensure_ascii=False, indent=2)

        log.info(f"Firma de línea base guardada: {FIRMA_FILENAME}")
        return str(ruta), None

    except PermissionError as e:
        log.error(f"Sin permisos para guardar firma: {e}")
        return None, f"Sin permisos: {e}"
    except Exception as e:
        log.error(f"Error guardando firma: {e}", exc_info=True)
        return None, str(e)


def cargar_firma_base(carpeta: str) -> tuple:
    """
    Carga la firma de línea base desde expediente_firmado.json.
    Retorna (dict_firma, error_str).
    """
    ruta = Path(carpeta) / FIRMA_FILENAME
    if not ruta.exists():
        return None, f"No se encontró {FIRMA_FILENAME} en {carpeta}"
    try:
        with open(ruta, "r", encoding="utf-8") as f:
            firma = json.load(f)
        log.info(f"Firma cargada: {firma.get('total_documentos',0)} documentos, "
                 f"timestamp: {firma.get('timestamp_firma','')}")
        return firma, None
    except json.JSONDecodeError as e:
        return None, f"Archivo de firma corrupto o modificado: {e}"
    except (FileNotFoundError, PermissionError) as e:
        return None, f"Sin acceso al archivo de firma: {e}"
    except Exception as e:
        return None, str(e)



# Prefijos de archivos generados por el sistema — excluir de auditoría
PREFIJOS_SISTEMA = ("INDICE_", "LOG_", "AUDITORIA_", "LOG_AUDITORIA_",
                    "LOG_COPIA_", "LOG_INDICE_", "expediente_firmado",
                    ".indice_cache", "~$")

def _es_archivo_sistema(nombre: str) -> bool:
    """Retorna True si el archivo fue generado por el sistema y debe excluirse."""
    n = nombre.upper()
    return any(n.startswith(p.upper()) for p in PREFIJOS_SISTEMA) or nombre.startswith("~$")

def verificar_firma_base(carpeta_foliados: str,
                          carpeta_firma: str = None,
                          callback_progreso=None) -> dict:
    """
    Verifica el estado actual del expediente contra la firma de línea base.
    Más liviano que auditar_expediente() — no requiere el Excel.
    Solo necesita expediente_firmado.json en la carpeta.

    Retorna dict con: resultados, resumen, integridad_global,
                      hash_global_original, hash_global_actual, error.
    """
    carpeta_firma = carpeta_firma or carpeta_foliados
    firma, err = cargar_firma_base(carpeta_firma)
    if err:
        return {"error": err, "resultados": [], "resumen": {},
                "integridad_global": False,
                "hash_global_original": "", "hash_global_actual": ""}

    # Construir mapa de referencia desde la firma
    hashes_ref = {
        d["nombre_foliado"]: d["hash_sha256"]
        for d in firma.get("documentos", [])
        if d.get("nombre_foliado") and len(d.get("hash_sha256","")) == 64
    }

    # Reutilizar el motor de auditoría existente construyendo un Excel temporal
    # en memoria — en realidad comparamos directamente contra hashes_ref
    carpeta = Path(carpeta_foliados)
    if not carpeta.exists():
        return {"error": f"Carpeta no encontrada: {carpeta_foliados}",
                "resultados": [], "resumen": {}, "integridad_global": False,
                "hash_global_original": "", "hash_global_actual": ""}

    archivos_actuales = {}
    for p in carpeta.rglob("*"):
        if p.is_file() and p.suffix.lower() in (EXT_ALL | EXT_SHP_COMP):
            if not _es_archivo_sistema(p.name):
                archivos_actuales[p.name] = p

    resultados = []; total = len(hashes_ref); procesados = 0

    for nombre, hash_original in hashes_ref.items():
        procesados += 1
        if callback_progreso:
            try: callback_progreso(procesados, total, nombre)
            except Exception: pass

        if nombre not in archivos_actuales:
            resultados.append({
                "nombre": nombre, "estado": "FALTANTE",
                "hash_original": hash_original,
                "hash_actual": "—", "ruta": None})
            continue

        ruta_v = archivos_actuales[nombre]
        hash_actual = generar_hash(ruta_v)
        estado      = "ÍNTEGRO" if hash_actual == hash_original else "MODIFICADO"
        fecha_mod   = ""
        if estado == "MODIFICADO":
            try:
                import os as _os
                ts = _os.path.getmtime(ruta_v)
                fecha_mod = datetime.fromtimestamp(ts).strftime("%d/%m/%Y %H:%M:%S")
            except Exception:
                fecha_mod = "—"
        resultados.append({
    ...
    # Desconocidos
    nombres_firma = set(hashes_ref.keys())
    for nombre, ruta in archivos_actuales.items():
        if nombre not in nombres_firma and ruta.suffix.lower() not in EXT_SHP_COMP:
            resultados.append({
                "nombre": nombre, "estado": "DESCONOCIDO",
                "hash_original": "—",
                "hash_actual": generar_hash(ruta), "ruta": ruta})

    # Hash global actual
    integros = [r for r in resultados if r["estado"] == "ÍNTEGRO"]
    hash_actual_global = ""
    if integros:
        h = hashlib.sha256()
        for r in sorted(integros, key=lambda x: x["nombre"]):
            h.update(f"{r['nombre']}|{r['hash_actual']}\n".encode("utf-8"))
        hash_actual_global = h.hexdigest()
    ...
    integridad_global = (
        resumen.get("MODIFICADO", 0) == 0 and
        resumen.get("FALTANTE",   0) == 0 and
        resumen.get("DESCONOCIDO",0) == 0
    )

    hash_original_global = firma.get("hash_global", "")

    log.info(f"Verificación firma base: {resumen} | "
             f"hash_global {'COINCIDE' if hash_original_global == hash_actual_global else 'DIFIERE'}")

    return {
        "resultados":          resultados,
        "resumen":             resumen,
        "integridad_global":   integridad_global,
        "hash_global_original":hash_original_global,
        "hash_global_actual":  hash_actual_global,
        "firma_timestamp":     firma.get("timestamp_firma", ""),
        "firma_expediente":    firma.get("expediente", ""),
        "firma_total":         firma.get("total_documentos", 0),
        "error":               None,
    }


def firma_existe(carpeta: str) -> bool:
    """Retorna True si existe expediente_firmado.json en la carpeta."""
    return (Path(carpeta) / FIRMA_FILENAME).exists()


def info_firma(carpeta: str) -> str:
    """Retorna string descriptivo del estado de la firma en la carpeta."""
    ruta = Path(carpeta) / FIRMA_FILENAME
    if not ruta.exists():
        return "Sin firma de línea base"
    try:
        with open(ruta, "r", encoding="utf-8") as f:
            d = json.load(f)
        return (f"Firmado el {d.get('fecha_firma','?')}  |  "
                f"{d.get('total_documentos','?')} documentos  |  "
                f"Hash: {d.get('hash_global_corto','?')}")
    except Exception:
        return "Firma ilegible"
...
REQUIREMENTS = """# GESTOR DE EXPEDIENTES DIGITALES
...
# ══════════════════════════════════════════════════════════════════════════════
# MÓDULO DE AUDITORÍA DE INTEGRIDAD (v7.1)
# Verifica si los archivos siguen siendo idénticos a los indexados
# comparando SHA-256 actual vs hash guardado en el Excel.
# Estados: ÍNTEGRO / MODIFICADO / FALTANTE / DESCONOCIDO
# ══════════════════════════════════════════════════════════════════════════════

AUDIT_COLORES = {
    "ÍNTEGRO":     {"bg": "E9F7EF", "fg": "1E8449"},
    "MODIFICADO":  {"bg": "FDEDEC", "fg": "C0392B"},
    "FALTANTE":    {"bg": "FDEDEC", "fg": "922B21"},
    "DESCONOCIDO": {"bg": "FEF9E7", "fg": "9A7D0A"},
}


def leer_hashes_desde_excel(ruta_excel: str) -> tuple:
    """
    Lee el Excel del índice y extrae nombre_foliado + hash SHA-256.
    Detecta la columna de hash buscando el encabezado automáticamente.
    Retorna (dict{nombre: hash}, hash_global_expediente, error_str).
    """
    if not XLSX_OK:
        return {}, "", "openpyxl no instalado"
    try:
        wb = openpyxl.load_workbook(ruta_excel, data_only=True)
        # Buscar hoja RESUMEN primero, luego primera hoja disponible
        ws = None
        for nombre_hoja in (["RESUMEN"] + list(wb.sheetnames)):
            if nombre_hoja in wb.sheetnames:
                ws = wb[nombre_hoja]
                break
        if ws is None:
            return {}, "", "No se encontró hoja de datos en el Excel"

        # Detectar fila de encabezado y columnas relevantes
        col_nombre = col_hash = fila_enc = None
        for fila in range(1, 5):
            for col in range(1, ws.max_column + 1):
                val = str(ws.cell(row=fila, column=col).value or "").strip().lower()
                if "nombre foliado" in val:
                    col_nombre = col; fila_enc = fila
                if "sha-256" in val or "sha256" in val or "hash" in val:
                    col_hash = col
            if col_nombre and col_hash:
                break

        if not col_nombre or not col_hash:
            return {}, "", (
                "No se encontraron las columnas 'Nombre foliado' y 'SHA-256'.\n"
                "Selecciona un Excel generado por este programa.")

        hashes = {}; hash_expediente = ""
        for fila in range(fila_enc + 1, ws.max_row + 1):
            nombre = str(ws.cell(row=fila, column=col_nombre).value or "").strip()
            hash_v = str(ws.cell(row=fila, column=col_hash).value or "").strip()
            if "hash global" in nombre.lower():
                hash_expediente = hash_v; continue
            # Saltar separadores de subcarpeta y filas vacías o sin hash válido
            if not nombre or len(hash_v) != 64:
                continue
            hashes[nombre] = hash_v

        log.info(f"Excel leído: {len(hashes)} registros con hash")
        return hashes, hash_expediente, None

    except PermissionError as e:
        return {}, "", f"Sin permisos para leer el Excel: {e}"
    except Exception as e:
        log.error(f"Error leyendo Excel auditoría: {e}", exc_info=True)
        return {}, "", str(e)


def auditar_expediente(ruta_excel: str, carpeta_foliados: str,
                        callback_progreso=None) -> dict:
    """
    Motor de auditoría de integridad. Independiente de la GUI.

    Args:
        ruta_excel:        Ruta al .xlsx generado previamente
        carpeta_foliados:  Carpeta con los archivos a verificar
        callback_progreso: fn(i, total, nombre) — opcional

    Returns:
        dict con: resultados, resumen, hashes globales, integridad_global, error
    """
    hashes_orig, hash_exp_orig, err = leer_hashes_desde_excel(ruta_excel)
    if err:
        return {"error": err, "resultados": [], "resumen": {},
                "integridad_global": False,
                "hash_expediente_original": "", "hash_expediente_actual": ""}
    ...
    # Recolectar archivos actuales en la carpeta (recursivo)
    # Se excluyen archivos generados por el propio sistema
    archivos_actuales = {}
    for p in carpeta.rglob("*"):
        if p.is_file() and p.suffix.lower() in (EXT_ALL | EXT_SHP_COMP):
            if not _es_archivo_sistema(p.name):
    ...
    resultados = []; total = len(hashes_orig); procesados = 0

    # Verificar cada archivo del índice
    for nombre, hash_original in hashes_orig.items():
        procesados += 1
        if callback_progreso:
            try: callback_progreso(procesados, total, nombre)
            except Exception: pass
        ...
        if nombre not in archivos_actuales:
            resultados.append({
                "nombre": nombre, "estado": "FALTANTE",
                "hash_original": hash_original, "hash_actual": "—", "ruta": None})
            continue

        ruta_actual = archivos_actuales[nombre]
        hash_actual = generar_hash(ruta_actual)
        estado = "ÍNTEGRO" if hash_actual == hash_original else "MODIFICADO"
        fecha_mod = ""
        if estado == "MODIFICADO":
            try:
                import os as _os
                ts = _os.path.getmtime(ruta_actual)
                fecha_mod = datetime.fromtimestamp(ts).strftime("%d/%m/%Y %H:%M:%S")
            except Exception:
                fecha_mod = "—"
        resultados.append({
    ...
    # Detectar archivos DESCONOCIDOS — en carpeta pero no en índice
    nombres_indice = set(hashes_orig.keys())
    for nombre, ruta in archivos_actuales.items():
        if nombre not in nombres_indice and ruta.suffix.lower() not in EXT_SHP_COMP:
            resultados.append({
                "nombre": nombre, "estado": "DESCONOCIDO",
                "hash_original": "—", "hash_actual": generar_hash(ruta), "ruta": ruta})

    # Hash global actual (solo sobre íntegros)
    docs_integros = [r for r in resultados if r["estado"] == "ÍNTEGRO"]
    hash_exp_actual = ""
    if docs_integros:
        h = hashlib.sha256()
        for r in sorted(docs_integros, key=lambda x: x["nombre"]):
            h.update(f"{r['nombre']}|{r['hash_actual']}\n".encode("utf-8"))
        hash_exp_actual = h.hexdigest()

    resumen = {}
    for r in resultados:
        resumen[r["estado"]] = resumen.get(r["estado"], 0) + 1

    integridad_global = (resumen.get("MODIFICADO", 0) == 0 and
                          resumen.get("FALTANTE", 0) == 0 and
                          resumen.get("DESCONOCIDO", 0) == 0)

    log.info(f"Auditoría completa: {resumen}")
    return {
        "resultados": resultados, "resumen": resumen,
        "hash_expediente_original": hash_exp_orig,
        "hash_expediente_actual":   hash_exp_actual,
        "integridad_global":        integridad_global,
        "error": None,
    }


def exportar_reporte_auditoria_excel(resultado: dict, carpeta: str) -> tuple:
    """Genera reporte de auditoría en Excel. Retorna (ruta, error)."""
    if not XLSX_OK: return None, "openpyxl no instalado"
    try:
        wb = openpyxl.Workbook(); ws = wb.active; ws.title = "AUDITORIA"

        ws.merge_cells("A1:E1"); c = ws["A1"]
        c.value = f"REPORTE DE AUDITORÍA DE INTEGRIDAD — {APP_VERSION}"
        c.font  = Font(name="Calibri", size=12, bold=True, color="FFFFFF")
        c.fill  = PatternFill(start_color="1A3A5C", end_color="1A3A5C", fill_type="solid")
        c.alignment = Alignment(horizontal="center"); ws.row_dimensions[1].height = 22

        ws.merge_cells("A2:E2"); c2 = ws["A2"]
        res = resultado["resumen"]; ts = datetime.now().strftime("%d/%m/%Y %H:%M")
        c2.value = (f"Generado: {ts}  |  "
                    f"OK:{res.get('INTEGRO',res.get('ÍNTEGRO',0))}  "
                    f"MOD:{res.get('MODIFICADO',0)}  "
                    f"FALT:{res.get('FALTANTE',0)}  "
                    f"DESC:{res.get('DESCONOCIDO',0)}")
        ...
        COLS  = ["Estado","Archivo","Hash original","Hash actual","Fecha modificación","Observación"]
        ANCH  = [14, 60, 22, 22, 20, 40]
        for ci, col in enumerate(COLS, 1):
            c = ws.cell(row=3, column=ci, value=col)
            c.font = Font(name="Calibri", size=9, bold=True, color="FFFFFF")
            c.fill = PatternFill(start_color="2C3E50", end_color="2C3E50", fill_type="solid")
            c.alignment = Alignment(horizontal="center")
            ws.column_dimensions[get_column_letter(ci)].width = ANCH[ci-1]
        ws.row_dimensions[3].height = 16

        def _h_p(h):
            if not h or h == "—" or len(h) < 16: return h or "—"
            return f"{h[:8]}...{h[-8:]}"

        for fi, r in enumerate(resultado["resultados"], 4):
            estado = r["estado"]
            col    = AUDIT_COLORES.get(estado, {"bg":"FFFFFF","fg":"000000"})
            obs = {"MODIFICADO": "ALERTA: archivo modificado desde la indexación",
                   "FALTANTE":   "ALERTA: archivo borrado o movido",
                   "DESCONOCIDO":"AVISO: no estaba en el índice original"}.get(estado,"")
            fecha_mod = r.get("fecha_modificacion", "")
            vals = [estado, r["nombre"], _h_p(r["hash_original"]),
                    _h_p(r["hash_actual"]), fecha_mod, obs]
            for ci, val in enumerate(vals, 1):
                c = ws.cell(row=fi, column=ci, value=val)
                c.font = Font(name="Calibri", size=8.5, bold=(ci==1),
                              color=col["fg"] if ci==1 else (
                              "C0392B" if ci==5 and fecha_mod else (
                              "C0392B" if obs.startswith("ALERTA") else "444444")))
                c.fill = PatternFill(start_color=col["bg"], end_color=col["bg"],
                                     fill_type="solid")
        ...
        fila_v = len(resultado["resultados"]) + 5
        ws.merge_cells(f"A{fila_v}:F{fila_v}"); cv = ws[f"A{fila_v}"]
        if resultado["integridad_global"]:
            cv.value = "EXPEDIENTE ÍNTEGRO — Todos los archivos coinciden con el índice original"
            cv.fill  = PatternFill(start_color="E9F7EF", end_color="E9F7EF", fill_type="solid")
            cv.font  = Font(name="Calibri", size=10, bold=True, color="1E8449")
        else:
            n_alertas = res.get("MODIFICADO",0) + res.get("FALTANTE",0)
            cv.value = f"EXPEDIENTE CON INCONSISTENCIAS — {n_alertas} archivo(s) requieren revisión"
            cv.fill  = PatternFill(start_color="FDEDEC", end_color="FDEDEC", fill_type="solid")
            cv.font  = Font(name="Calibri", size=10, bold=True, color="C0392B")
        cv.alignment = Alignment(horizontal="center"); ws.row_dimensions[fila_v].height = 20
        ws.freeze_panes = "A4"
        ws.auto_filter.ref = f"A3:E{len(resultado['resultados'])+3}"

        nom  = f"AUDITORIA_INTEGRIDAD_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        ruta = Path(carpeta) / nom
        wb.save(str(ruta)); log.info(f"Reporte auditoría: {nom}")
        return str(ruta), None
    except PermissionError as e: return None, f"Sin permisos: {e}"
    except Exception as e:
        log.error(f"Error reporte auditoría: {e}", exc_info=True); return None, str(e)


def exportar_reporte_auditoria_log(resultado: dict, carpeta: str) -> str:
    """Genera log .txt del reporte de auditoría. Retorna ruta."""
    nom  = f"LOG_AUDITORIA_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ruta = Path(carpeta) / nom
    try:
        with open(ruta, "w", encoding="utf-8") as f:
            f.write("="*70 + "\n")
            f.write(f"REPORTE DE AUDITORÍA — {APP_VERSION}\n")
            f.write(f"Generado: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}\n")
            f.write("="*70 + "\n\n")
            res = resultado["resumen"]
            f.write("RESUMEN:\n")
            for estado, cnt in sorted(res.items()):
                f.write(f"  {estado:<15} {cnt:>4}\n")
            f.write(f"  {'Total':<15} {len(resultado['resultados']):>4}\n\n")
            h_o = resultado.get("hash_expediente_original","")
            h_a = resultado.get("hash_expediente_actual","")
            f.write(f"HASH GLOBAL DEL EXPEDIENTE:\n")
            f.write(f"  Original: {h_o or '—'}\n")
            f.write(f"  Actual:   {h_a or '—'}\n")
            if h_o and h_a:
                f.write(f"  Estado:   {'COINCIDE' if h_o==h_a else 'DIFIERE'}\n")
            f.write(f"\nVEREDICTO: {'EXPEDIENTE INTEGRO' if resultado['integridad_global'] else 'EXPEDIENTE CON INCONSISTENCIAS'}\n\n")
            f.write("="*70 + "\n")
            f.write("DETALLE:\n" + "="*70 + "\n")
            for r in sorted(resultado["resultados"], key=lambda x: x["estado"]):
                f.write(f"\n[{r['estado']}] {r['nombre']}\n")
                f.write(f"  Hash original: {r['hash_original']}\n")
                f.write(f"  Hash actual:   {r['hash_actual']}\n")
                if r.get("fecha_modificacion"):
                    f.write(f"  Fecha modif.:  {r['fecha_modificacion']} ← MODIFICADO\n")
        log.info(f"Log auditoría: {nom}")
        return str(ruta)
    except Exception as e:
        log.error(f"Error log auditoría: {e}"); return ""


def exportar_reporte_auditoria_word(resultado: dict, carpeta: str,
                                     config_extra: dict = None) -> tuple:
    """
    Genera reporte formal de auditoría en Word (.docx).
    Con membrete institucional, tabla coloreada y veredicto oficial.
    Retorna (ruta_str, error_str).
    """
    if not DOCX_OK:
    ...
    try:
        ...
        ts_nom= datetime.now().strftime("%Y%m%d_%H%M%S")
        n_ok  = res.get("ÍNTEGRO", 0)
        n_mod = res.get("MODIFICADO", 0)
        n_fal = res.get("FALTANTE", 0)
        n_des = res.get("DESCONOCIDO", 0)
        integro = resultado["integridad_global"]
        ...
        r0.font.color.rgb = RGBColor(0x1A, 0x3A, 0x5C)
        ...
        # ── Título del reporte ────────────────────────────────────
        tit = doc.add_heading("", level=1)
        tit.alignment = WD_ALIGN_PARAGRAPH.CENTER
        rt = tit.add_run("REPORTE DE AUDITORÍA DE INTEGRIDAD DOCUMENTAL")
        rt.font.size = Pt(12); rt.font.bold = True
        rt.font.color.rgb = RGBColor(0x1A, 0x3A, 0x5C)

        # ── Metadatos del reporte ─────────────────────────────────
        pm = doc.add_paragraph()
        pm.alignment = WD_ALIGN_PARAGRAPH.CENTER
        bits = []
        if cfg.get("expediente"): bits.append(f"Expediente: {cfg['expediente']}")
        if cfg.get("seccion"):    bits.append(f"Sección: {cfg['seccion']}")
        bits += [f"Total verificado: {total} archivos",
                 f"Fecha auditoría: {ts}",
                 f"Versión: {APP_VERSION}"]
        rm = pm.add_run("  |  ".join(bits))
        rm.font.size = Pt(8.5)
        rm.font.color.rgb = RGBColor(0x7F, 0x8C, 0x8D)

        doc.add_paragraph()

        # ── Recuadro de veredicto ─────────────────────────────────
        t_verd = doc.add_table(rows=1, cols=1)
        t_verd.style = "Table Grid"
        cell_v = t_verd.rows[0].cells[0]
        cell_v.width = Cm(16)
        ...
        if integro:
            texto_verd = "EXPEDIENTE ÍNTEGRO"
            detalle_verd = f"Los {n_ok} archivos verificados coinciden exactamente con el índice original."
            color_verd = RGBColor(0x1E, 0x84, 0x49)
            bg_verd    = "E9F7EF"
        else:
            texto_verd = "EXPEDIENTE CON INCONSISTENCIAS"
            partes = []
            if n_mod: partes.append(f"{n_mod} modificado(s)")
            if n_fal: partes.append(f"{n_fal} faltante(s)")
            if n_des: partes.append(f"{n_des} desconocido(s)")
            detalle_verd = "Se detectaron: " + ", ".join(partes) + ". Requiere revisión."
            color_verd = RGBColor(0xC0, 0x39, 0x2B)
            bg_verd    = "FDEDEC"

        rv = p_v.add_run(texto_verd)
        rv.font.size = Pt(14); rv.font.bold = True
        rv.font.color.rgb = color_verd

        p_v2 = cell_v.add_paragraph()
        p_v2.alignment = WD_ALIGN_PARAGRAPH.CENTER
        p_v2.paragraph_format.space_after = Pt(6)
        rv2 = p_v2.add_run(detalle_verd)
        rv2.font.size = Pt(9)
        rv2.font.color.rgb = color_verd

        # Color de fondo del veredicto
        from docx.oxml.ns import qn as _qn
        from docx.oxml import OxmlElement as _OxmlElement
        tc = cell_v._tc; tcPr = tc.get_or_add_tcPr()
        shd = _OxmlElement('w:shd')
        shd.set(_qn('w:val'),   'clear')
        shd.set(_qn('w:color'), 'auto')
        shd.set(_qn('w:fill'),  bg_verd)
        tcPr.append(shd)
        ...
        h2 = doc.add_heading("", level=2)
        rh2 = h2.add_run("RESUMEN POR ESTADO")
        rh2.font.size = Pt(10); rh2.font.bold = True
        rh2.font.color.rgb = RGBColor(0x1A, 0x3A, 0x5C)
        ...
        # Encabezado resumen
        for i, txt in enumerate(["Estado","Cantidad","% del total","Significado"]):
            _w_celda(t_res.rows[0], i, txt, bold=True, size=9,
                     align=C, bg="1A3A5C", fg="FFFFFF")

        ESTADOS_INFO = [
            ("ÍNTEGRO",      n_ok,  "E9F7EF", "1E8449", "Hash coincide — archivo sin alteraciones"),
            ("MODIFICADO",   n_mod, "FDEDEC", "C0392B", "Hash difiere — archivo posiblemente alterado"),
            ("FALTANTE",     n_fal, "FDEDEC", "922B21", "No encontrado en la carpeta verificada"),
            ("DESCONOCIDO",  n_des, "FEF9E7", "9A7D0A", "Presente en carpeta pero no en el índice"),
        ]
        for estado, cnt, bg, fg, sig in ESTADOS_INFO:
            fila = t_res.add_row()
            pct  = f"{cnt/total*100:.1f}%" if total > 0 else "0%"
            for i, (val, aln) in enumerate(zip(
                [estado, str(cnt), pct, sig],
                [C, C, C, WD_ALIGN_PARAGRAPH.LEFT]
            )):
                _w_celda(fila, i, val, bold=(i==0), size=9,
                         align=aln, bg=bg if cnt > 0 else "F8F9FA",
                         fg=fg if cnt > 0 else "95A5A6")

        doc.add_paragraph()

        # ── Hash global del expediente ────────────────────────────
        h3 = doc.add_heading("", level=2)
        rh3 = h3.add_run("INTEGRIDAD DEL CONJUNTO")
        rh3.font.size = Pt(10); rh3.font.bold = True
        rh3.font.color.rgb = RGBColor(0x1A, 0x3A, 0x5C)

        h_orig = resultado.get("hash_expediente_original", "")
        h_act  = resultado.get("hash_expediente_actual", "")
        coincide = (h_orig and h_act and h_orig == h_act)

        t_hash = doc.add_table(rows=3, cols=2)
        t_hash.style = "Table Grid"
        datos_hash = [
            ("Hash global original", h_orig or "No disponible en el Excel"),
            ("Hash global actual",   h_act  or "No calculado"),
            ("Estado del conjunto",  "COINCIDE — El expediente no fue alterado"
                                     if coincide else
                                     "DIFIERE — El conjunto cambió respecto al índice original"
                                     if h_orig and h_act else "—"),
        ]
        colores_hash = [
            ("EBF5FB", "1A3A5C"), ("EBF5FB", "1A3A5C"),
            ("E9F7EF" if coincide else "FDEDEC",
             "1E8449" if coincide else "C0392B"),
        ]
        for i, ((etiq, val), (bg, fg)) in enumerate(zip(datos_hash, colores_hash)):
            _w_celda(t_hash.rows[i], 0, etiq, bold=True, size=8.5,
                     align=WD_ALIGN_PARAGRAPH.LEFT, bg=bg, fg="1A3A5C")
            _w_celda(t_hash.rows[i], 1, val,  bold=False, size=7.5,
                     align=WD_ALIGN_PARAGRAPH.LEFT, bg=bg, fg=fg)

        doc.add_paragraph()

        # ── Tabla detalle por archivo ─────────────────────────────
        h4 = doc.add_heading("", level=2)
        rh4 = h4.add_run("DETALLE POR ARCHIVO")
        rh4.font.size = Pt(10); rh4.font.bold = True
        rh4.font.color.rgb = RGBColor(0x1A, 0x3A, 0x5C)

        def _h_p(h):
            if not h or h == "—" or len(h) < 16: return h or "—"
            return f"{h[:10]}...{h[-6:]}"

        # Ordenar: primero problemas, luego íntegros
        orden = {"MODIFICADO":0,"FALTANTE":1,"DESCONOCIDO":2,"ÍNTEGRO":3}
        resultados_ord = sorted(resultado["resultados"],
                                key=lambda x: orden.get(x["estado"], 9))

        t_det = doc.add_table(rows=1, cols=4)
        t_det.style = "Table Grid"
        ANCHOS_DET = [3.5, 8.5, 3.2, 3.2]
        for i, ancho in enumerate(ANCHOS_DET):
            for cell in t_det.columns[i].cells:
                cell.width = Cm(ancho)
        for i, txt in enumerate(["Estado","Archivo","Hash original","Hash actual"]):
            _w_celda(t_det.rows[0], i, txt, bold=True, size=8,
                     align=WD_ALIGN_PARAGRAPH.CENTER, bg="2C3E50", fg="FFFFFF")

        COLORES_DET = {
            "ÍNTEGRO":     ("EBF5FB","1E8449"),
            "MODIFICADO":  ("FDEDEC","C0392B"),
            "FALTANTE":    ("FDEDEC","922B21"),
            "DESCONOCIDO": ("FEF9E7","9A7D0A"),
        }
        for r in resultados_ord:
            estado = r["estado"]
            bg, fg = COLORES_DET.get(estado, ("FFFFFF","000000"))
            fila = t_det.add_row()
            vals = [estado, r["nombre"],
                    _h_p(r["hash_original"]),
                    _h_p(r["hash_actual"])]
            alns = [WD_ALIGN_PARAGRAPH.CENTER,
                    WD_ALIGN_PARAGRAPH.LEFT,
                    WD_ALIGN_PARAGRAPH.CENTER,
                    WD_ALIGN_PARAGRAPH.CENTER]
            for i, (val, aln) in enumerate(zip(vals, alns)):
                _w_celda(fila, i, val, bold=(i==0), size=7.5,
                         align=aln, bg=bg,
                         fg=fg if i == 0 else "000000")
        ...
        rp = pp.add_run(
            f"{inst_pie()}  |  {APP_VERSION}  |  "
            f"Auditoría generada: {ts}")
        rp.font.size = Pt(7)
        rp.font.color.rgb = RGBColor(0x95, 0xA5, 0xA6)

        # ── Guardar ───────────────────────────────────────────────
        nom     = f"AUDITORIA_INTEGRIDAD_{ts_nom}.docx"
        ruta    = Path(carpeta) / nom
        doc.save(str(ruta))
        log.info(f"Reporte Word auditoría: {nom}")
        return str(ruta), None

    except PermissionError as e:
        log.error(f"Sin permisos Word auditoría: {e}")
        return None, f"Sin permisos: {e}"
    except Exception as e:
        log.error(f"Error Word auditoría: {e}", exc_info=True)
        return None, str(e)


class TestMotorAnalisis(unittest.TestCase):
    """
    Pruebas automáticas — Gestor de Expedientes Digitales
    ...
    def test_asignar_folios_inicio_1(self):
        regs=[DocumentoAnalizado(tipo="PDF",fecha="",hora="",descripcion="d",extension=".pdf")
               for _ in range(3)]
    ...
    def test_integracion_asignar_folios_y_hash(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(exp.registros[0].folio,"010")
            hash_global=generar_hash_expediente(exp.registros)
            self.assertEqual(len(hash_global),64)
...
class _Tooltip:
    """Tooltip flotante para Treeview — muestra info de trazabilidad."""
    ...
    def mostrar(self, texto: str, x: int, y: int):
        ...
        self.ventana.wm_geometry(f"+{x+14}+{y+10}")
        fr = tk.Frame(self.ventana, bg="#1a2535",
                      relief="solid", bd=1)
        fr.pack()
        tk.Label(fr, text=texto, bg="#1a2535", fg="#a8c8f0",
...
class InterfazGrafica(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title(f"Gestor de Expedientes Digitales — {inst_subtitulo()}")
    ...
    def _construir_ui(self):
        ...
        # Izquierda: nombre de la institución desde JSON (prominente)
        self.lbl_inst_nombre=tk.Label(fh,
                 text=f"📋  {inst_nombre()}",
                 font=("Segoe UI",13,"bold"),bg=CP,fg=CA)
        self.lbl_inst_nombre.pack(side="left",padx=20)
        # Subtítulo debajo del nombre
        self.lbl_inst_sub=tk.Label(fh,
                 text=inst_subtitulo(),
                 font=("Segoe UI",9),bg=CP,fg="#bdc3c7")
        self.lbl_inst_sub.pack(side="left",padx=(0,16))
        # Derecha: versión + botón institución
        self.lbl_inst=tk.Label(fh,
                 text=APP_VERSION,
                 font=("Segoe UI",9),bg=CP,fg="#7f8c8d")
        ...
        self.tree.bind("<Motion>",  self._tooltip_motion)
        self.tree.bind("<Leave>",   lambda e: self._tooltip_ocultar())
        ...
        self.lbl_estado=tk.Label(fb,text="",
            bg=CP,fg="#7f8c8d",font=("Segoe UI",10,"bold"),width=20,anchor="w")
        self.lbl_estado.pack(side="left",padx=(8,2))
        self._btn(fb,"🗑",self._limpiar,CR).pack(side="left",padx=(0,6))
        ...
        self.lbl_firma=tk.Label(fb,text="",
            bg=CP,fg="#ECF0F1",font=("Segoe UI",8,"bold"))
        self.lbl_firma.pack(side="left",padx=4)
        self._btn(fb,"🔏 Firmar",    self._firmar,   "#8E44AD").pack(side="right",padx=3)
        self._btn(fb,"🔍 Auditar",   self._auditar,  "#16A085").pack(side="right",padx=3)
        self._btn(fb,"📁 Foliados",  self._copiar,   CN).pack(side="right",padx=3)
        self._btn(fb,"📝 Word",      self._gen_word, CV).pack(side="right",padx=3)
        self._btn(fb,"📊 Excel",     self._gen_excel,CA).pack(side="right",padx=3)
        self._btn(fb,"📝+📊 Ambos", self._gen_ambos,CM).pack(side="right",padx=3)

    def _run_tests(self):
        win=tk.Toplevel(self); win.title("Pruebas Automáticas — v7.1")
    ...
    def _menu_contextual(self, event):
        ...
        menu.add_separator()
        menu.add_command(label="🔬 Ver auditoría detallada",
                         command=lambda:self._panel_auditoria_detalle(item))
    ...
    def _actualizar_titulo_ventana(self):
        """Actualiza el título de la ventana con la institución del JSON."""
        self.title(f"Gestor de Expedientes Digitales — {inst_subtitulo()}")

    def _actualizar_encabezado_inst(self):
        self._actualizar_titulo_ventana()
        try:
            self.lbl_inst_nombre.config(text=f"📋  {inst_nombre()}")
            self.lbl_inst_sub.config(text=inst_subtitulo())
        except Exception: pass
    ...
    def _tooltip_ocultar(self):
        if self._tooltip:
            self._tooltip.ocultar()

    def _tooltip_motion(self, event):
        """Muestra tooltip con trazabilidad al pasar el mouse sobre una fila."""
        item = self.tree.identify_row(event.y)
        col  = self.tree.identify_column(event.x)
        if not item or item.startswith("SEP_"):
            self._tooltip_ocultar(); return
        ...
        if not self.expediente or idx < 0 or idx >= len(self.expediente.registros):
            self._tooltip_ocultar(); return

        r = self.expediente.registros[idx]

        # Solo mostrar tooltip en columnas relevantes
        COLS_UI = ("N° Folio","N° Doc.","Subcarpeta","Fecha","Hora","Tipo",
                   "Descripción / Asunto","Págs","Tamaño",
                   "F.Fecha","Conf.","SHA-256","Nombre foliado")
        try:
            col_idx  = int(col.replace("#","")) - 1
            col_name = COLS_UI[col_idx] if col_idx < len(COLS_UI) else ""
        except (ValueError, IndexError):
            col_name = ""

        # Construir texto del tooltip según la columna
        if col_name in ("Conf.", "Tipo", "F.Fecha"):
            conf    = r.get("conf_tipo","?")
            fuente_t= r.get("fuente_tipo","?")
            regla_t = r.get("regla_tipo","?")
            fuente_f= r.get("fuente_fecha","?")
            regla_f = r.get("regla_fecha","?")
            num_doc = r.get("num_doc","—")
            regla_n = r.get("regla_num_doc","—")
            if hasattr(r,"fue_editado"):
                editado = "Sí" if r.fue_editado() else "No"
            else:
                editado = "Sí" if r.get("historial_ediciones",[]) else "No"

            txt = "\n".join([
                f"TIPO: " + str(r.get("tipo","?")),
                "  Fuente:   " + str(fuente_t),
                "  Regla:    " + str(regla_t),
                "  Conf:     " + str(conf),
                "",
                "FECHA: " + str(r.get("fecha","?")) + " " + str(r.get("hora","")),
                "  Fuente:   " + str(fuente_f),
                "  Regla:    " + str(regla_f),
                "",
                "N° DOC: " + str(num_doc),
        ...
        elif col_name == "SHA-256":
            h = r.get("hash_sha256","") if isinstance(r, dict) else getattr(r,"hash_sha256","")
            txt = "SHA-256 completo:\n" + str(h or "—")
        elif col_name == "Nombre foliado":
            nom_f = r.get("nombre_foliado","") if isinstance(r,dict) else getattr(r,"nombre_foliado","")
            txt = "Nombre foliado completo:\n" + str(nom_f)
        else:
            self._tooltip_ocultar(); return

        if not self._tooltip:
            self._tooltip = _Tooltip(self.tree)
        x_abs = self.tree.winfo_rootx() + event.x
        y_abs = self.tree.winfo_rooty() + event.y
        self._tooltip.mostrar(txt, x_abs, y_abs)

    def _calcular_estado_expediente(self, regs: list) -> tuple:
        """
        Calcula el estado global del expediente.
        Retorna (icono, texto, color) según problemas detectados.

        🔴 INCONSISTENTE  → gaps en folios, duplicados o archivos sin hash válido
        🟡 CON OBSERVACIONES → expediente sin firma o firma no coincide
        🟢 VALIDADO        → foliación correcta, hashes válidos y firma vigente
        """
        problemas = validar_foliacion(regs)
        sin_hash  = len(self.expediente.archivos_sin_hash)

        # Problemas reales de integridad → INCONSISTENTE
        if problemas or sin_hash > 0:
            return "🔴", "INCONSISTENTE", "#e74c3c"

        # Verificar estado de firma
        carpeta = str(self.expediente.carpeta)
        if not firma_existe(carpeta):
            return "🟢", "VALIDADO", "#27ae60"

        # Comparar hash actual vs firma guardada
        try:
            firma, err = cargar_firma_base(carpeta)
            if err or not firma:
                return "🟢", "VALIDADO", "#27ae60"
            hash_actual = generar_hash_expediente(
                [r for r in regs if hasattr(r, "hash_sha256")])
            hash_firmado = firma.get("hash_global", "")
            if hash_actual != hash_firmado:
                return "🟡", "CON OBSERVACIONES", "#f39c12"
        except Exception:
            return "🟢", "VALIDADO", "#27ae60"
    ...
    def _fin(self):
        ...
        sin_hash=len(self.expediente.archivos_sin_hash)
        subs=len(set(r["subcarpeta"] for r in regs))

        # Indicador de estado global
        icono_estado, txt_estado, color_estado = self._calcular_estado_expediente(regs)
        self.lbl_estado.config(
            text=f"{icono_estado} {txt_estado}",
            fg=color_estado)
    ...
    def _editar(self,event):
        ...
        def upd(*_):
            from dataclasses import asdict as _asdict
            try:
                r_tmp = {k:v for k,v in _asdict(r).items() if k != "ruta"}
            except Exception:
                r_tmp = {k: getattr(r, k, "") for k in vars_e}
            r_tmp["ruta"] = getattr(r, "ruta", None)
    ...
    def _auditar(self):
        """
        Módulo de Auditoría de Integridad.
        Carga un Excel del índice generado previamente y verifica
        si los archivos en la carpeta Foliados siguen siendo idénticos.
        """
        # Paso 1: seleccionar el Excel del índice
        ruta_excel = filedialog.askopenfilename(
            title="Selecciona el Excel del índice generado",
            filetypes=[("Excel","*.xlsx"),("Todos","*.*")])
        if not ruta_excel:
            return

        # Paso 2: seleccionar la carpeta con los archivos a verificar
        carpeta = filedialog.askdirectory(
            title="Selecciona la carpeta con los archivos (ej: Foliados/)")
        if not carpeta:
            return

        # Paso 3: ejecutar auditoría en hilo para no bloquear UI
        win = tk.Toplevel(self)
        win.title("🔍 Auditoría de Integridad — Procesando...")
        win.configure(bg=CF); win.geometry("900x560"); win.grab_set()

        tk.Label(win, text="🔍  AUDITORÍA DE INTEGRIDAD",
                 font=("Segoe UI",13,"bold"), bg=CF, fg="#16A085").pack(pady=(16,4))
        # Mostrar estado de firma si existe
        info_f = info_firma(carpeta)
        color_f = "#8E44AD" if "Firmado" in info_f else "#7f8c8d"
        tk.Label(win, text=f"🔏 {info_f}",
                 font=("Segoe UI",8,"italic"), bg=CF, fg=color_f).pack()
        tk.Label(win,
                 text=f"Excel: {Path(ruta_excel).name}  |  Carpeta: {Path(carpeta).name}",
                 font=("Segoe UI",8), bg=CF, fg="#7f8c8d").pack()

        progreso = ttk.Progressbar(win, orient="horizontal", mode="determinate")
        progreso.pack(fill="x", padx=20, pady=8)
        lbl_prog = tk.Label(win, text="Iniciando...", bg=CF, fg=CT,
                             font=("Segoe UI",9))
        lbl_prog.pack()

        # Tabla de resultados
        fm = tk.Frame(win, bg=CF)
        fm.pack(fill="both", expand=True, padx=16, pady=(4,4))

        COLS_A = ("Estado","Archivo","Hash original","Hash actual","Fecha modificación")
        ANCH_A = [100, 340, 130, 130, 150]
        tree_a = ttk.Treeview(fm, columns=COLS_A, show="headings", height=12)
        for col, ancho in zip(COLS_A, ANCH_A):
            tree_a.heading(col, text=col)
            tree_a.column(col, width=ancho, anchor="w")
        sx_a = ttk.Scrollbar(fm, orient="horizontal", command=tree_a.xview)
        sy_a = ttk.Scrollbar(fm, orient="vertical",   command=tree_a.yview)
        tree_a.configure(xscrollcommand=sx_a.set, yscrollcommand=sy_a.set)
        tree_a.grid(row=0, column=0, sticky="nsew")
        sy_a.grid(row=0, column=1, sticky="ns")
        sx_a.grid(row=1, column=0, sticky="ew")
        fm.grid_rowconfigure(0, weight=1); fm.grid_columnconfigure(0, weight=1)

        # Tags de color por estado
        tree_a.tag_configure("ÍNTEGRO",     background="#E9F7EF", foreground="#1E8449")
        tree_a.tag_configure("MODIFICADO",  background="#FDEDEC", foreground="#C0392B")
        tree_a.tag_configure("FALTANTE",    background="#FDEDEC", foreground="#922B21")
        tree_a.tag_configure("DESCONOCIDO", background="#FEF9E7", foreground="#9A7D0A")

        # Barra de resumen y botones (inicialmente ocultos)
        fb_a = tk.Frame(win, bg=CP, pady=8); fb_a.pack(fill="x", side="bottom")
        lbl_resumen = tk.Label(fb_a, text="", bg=CP, fg=CT, font=("Segoe UI",9))
        lbl_resumen.pack(side="left", padx=12)

        def _exportar_word_a():
            cfg_extra = {}
            if self.expediente:
                cfg_extra["expediente"] = self.expediente_var.get().strip()
                cfg_extra["seccion"]    = self.seccion_var.get().strip()
            ruta_w, err = exportar_reporte_auditoria_word(
                resultado_final[0], carpeta, cfg_extra)
            if err:
                messagebox.showerror("Error", err, parent=win); return
            messagebox.showinfo("Word generado",
                "Reporte Word de auditoría:\n" + Path(ruta_w).name +
                "\n\nGuardado en:\n" + str(carpeta), parent=win)
            try: os.startfile(ruta_w)
            except Exception: pass

        def _exportar_xlsx():
            ruta_r, err = exportar_reporte_auditoria_excel(resultado_final[0], carpeta)
            if err:
                messagebox.showerror("Error", err, parent=win); return
            messagebox.showinfo("Reporte generado",
                "Excel de auditoría:\n" + Path(ruta_r).name +
                "\n\nGuardado en:\n" + str(carpeta), parent=win)
            try: os.startfile(ruta_r)
            except Exception: pass

        def _exportar_log_a():
            ruta_l = exportar_reporte_auditoria_log(resultado_final[0], carpeta)
            if ruta_l:
                messagebox.showinfo("Log generado",
                    "Log de auditoría:\n" + Path(ruta_l).name, parent=win)

        def _verificar_firma_json():
            """Verificar contra firma JSON en lugar del Excel."""
            if not firma_existe(carpeta):
                messagebox.showinfo("Sin firma",
                    f"No existe {FIRMA_FILENAME} en la carpeta.\n\n"
                    "Primero genera el índice y presiona 🔏 Firmar.",
                    parent=win); return
            self.lbl.config(text="⏳ Verificando firma de línea base...")
            def _hilo_firma():
                def _cb(i,t,n): win.after(0,lambda i=i,t=t,n=n:[
                    progreso.configure(maximum=t,value=i),
                    lbl_prog.config(text=f"Verificando {i}/{t}: {n}")])
                res_f = verificar_firma_base(carpeta,carpeta,_cb)
                resultado_final[0] = res_f
                win.after(0,lambda:_mostrar_resultado(res_f))
            threading.Thread(target=_hilo_firma,daemon=True).start()

        btn_firma_json = self._btn(fb_a,"🔏 Verificar firma",
                                   _verificar_firma_json,"#8E44AD")
        btn_firma_json.pack(side="left",padx=6)
        btn_word= self._btn(fb_a, "📝 Word oficial",   _exportar_word_a, CV)
        btn_word.pack(side="right", padx=4)
        btn_xls = self._btn(fb_a, "📊 Exportar Excel", _exportar_xlsx, CA)
        btn_xls.pack(side="right", padx=6)
        btn_log = self._btn(fb_a, "📄 Exportar Log",   _exportar_log_a, CN)
        btn_log.pack(side="right", padx=4)
        btn_xls.config(state="disabled"); btn_log.config(state="disabled")
        btn_word.config(state="disabled")

        resultado_final = [None]

        def _hilo_auditoria():
            def _cb(i, total, nombre):
                win.after(0, lambda i=i, t=total, n=nombre: [
                    progreso.configure(maximum=t, value=i),
                    lbl_prog.config(text=f"Verificando {i}/{t}: {n}")])

            resultado = auditar_expediente(ruta_excel, carpeta, _cb)
            resultado_final[0] = resultado
            win.after(0, lambda: _mostrar_resultado(resultado))

        def _mostrar_resultado(resultado):
            lbl_prog.config(text="Análisis completado.")
            progreso.configure(value=progreso.cget("maximum") or 1)

            if resultado.get("error"):
                messagebox.showerror("Error en auditoría",
                    resultado["error"], parent=win)
                win.destroy(); return

            # Poblar tabla
            tree_a.delete(*tree_a.get_children())
            for r in sorted(resultado["resultados"],
                            key=lambda x: (x["estado"], x["nombre"])):
                def _h_p(h):
                    if not h or h == "—" or len(h) < 16: return h or "—"
                    return f"{h[:8]}...{h[-8:]}"
                tag = r["estado"]
                iconos = {"ÍNTEGRO":"✅","MODIFICADO":"⚠️","FALTANTE":"❌","DESCONOCIDO":"❓"}
                icono  = iconos.get(r["estado"], "")
                tree_a.insert("", "end", tags=(tag,),
                    values=(f"{icono} {r['estado']}", r["nombre"],
                            _h_p(r["hash_original"]), _h_p(r["hash_actual"]),
                            r.get("fecha_modificacion", "")))

            # Resumen en barra inferior
            res = resultado["resumen"]
            n_ok   = res.get("ÍNTEGRO", 0)
            n_mod  = res.get("MODIFICADO", 0)
            n_falt = res.get("FALTANTE", 0)
            n_desc = res.get("DESCONOCIDO", 0)

            if resultado["integridad_global"]:
                resumen_txt = f"✅ EXPEDIENTE ÍNTEGRO — {n_ok} archivos verificados correctamente"
                lbl_resumen.config(fg="#27ae60")
            else:
                resumen_txt = (f"⚠️ {n_mod} modificados  ❌ {n_falt} faltantes  "
                               f"❓ {n_desc} desconocidos  ✅ {n_ok} íntegros")
                lbl_resumen.config(fg="#e74c3c")

            lbl_resumen.config(text=resumen_txt)
            win.title(f"🔍 Auditoría — {'ÍNTEGRO' if resultado['integridad_global'] else 'INCONSISTENCIAS'}")

            # Habilitar exportación
            btn_xls.config(state="normal"); btn_log.config(state="normal")
            btn_word.config(state="normal")

            # Alerta si hay problemas graves
            if n_mod > 0 or n_falt > 0:
                messagebox.showwarning(
                    "Inconsistencias detectadas",
                    f"Se detectaron {n_mod + n_falt} problema(s):\n\n"
                    f"  {n_mod} archivo(s) MODIFICADOS\n"
                    f"  {n_falt} archivo(s) FALTANTES\n\n"
                    "Exporta el reporte Excel para documentar la auditoria.",
                    parent=win)

        threading.Thread(target=_hilo_auditoria, daemon=True).start()


    def _firmar(self):
        """
        Guarda la firma de línea base del expediente actual.
        Crea expediente_firmado.json en la carpeta del expediente.
        """
        if not self.expediente or not self.expediente.registros:
            messagebox.showinfo("Sin datos",
                "Primero analiza una carpeta antes de firmar."); return

        docs   = self.expediente.registros
        carpeta= str(self.expediente.carpeta)
        cfg    = self._config() or {}

        # Advertir si ya existe una firma previa
        if firma_existe(carpeta):
            info = info_firma(carpeta)
            resp = messagebox.askyesno(
                "Firma existente",
                f"Ya existe una firma de línea base:\n\n"
                f"{info}\n\n"
                f"¿Deseas reemplazarla con el estado actual?")
            if not resp: return

        # Verificar que todos los archivos tienen hash
        sin_hash = self.expediente.archivos_sin_hash
        if sin_hash:
            resp = messagebox.askyesno(
                "Archivos sin hash",
                f"{len(sin_hash)} archivo(s) no tienen hash calculado.\n"
                f"La firma será incompleta.\n\n¿Continuar de todas formas?")
            if not resp: return

        ruta, err = guardar_firma_base(docs, carpeta, cfg)
        if err:
            messagebox.showerror("Error al firmar", err); return

        # Actualizar etiqueta con estado de firma
        self._actualizar_lbl_firma()

        messagebox.showinfo(
            "Firma guardada",
            f"Línea base firmada correctamente.\n\n"
            f"Archivo: {FIRMA_FILENAME}\n"
            f"Documentos: {len(docs)}\n"
            f"Carpeta: {carpeta}\n\n"
            f"Este archivo es la referencia oficial para auditorías futuras.\n"
            f"Guárdalo junto a la carpeta Foliados.")

    def _actualizar_lbl_firma(self):
        """Actualiza la etiqueta de estado de firma en la barra inferior."""
        if not self.expediente: return
        info = info_firma(str(self.expediente.carpeta))
        self.lbl_firma.config(
            text=f"🔏 {info}",
            fg="#8E44AD" if "Firmado" in info else "#7f8c8d")


    def _panel_auditoria_detalle(self, iid: str):
        """
        Panel de auditoría detallada para un archivo específico.
        Muestra reglas aplicadas, trazabilidad completa y observaciones.
        """
        ...
        # Obtener valores compatibles con dict y dataclass
        def _g(campo, defecto="—"):
            if isinstance(r, dict): return r.get(campo, defecto) or defecto
            return getattr(r, campo, defecto) or defecto

        win = tk.Toplevel(self)
        win.title(f"Auditoría detallada — {_g('nombre')}")
        win.configure(bg=CF); win.geometry("740x560"); win.grab_set()

        # ── Encabezado ────────────────────────────────────────────
        tk.Label(win, text="🔬  AUDITORÍA DETALLADA DEL DOCUMENTO",
                 font=("Segoe UI",12,"bold"), bg=CF, fg=CA).pack(pady=(14,2))
        tk.Label(win, text=_g("nombre"),
                 font=("Segoe UI",9), bg=CF, fg="#7f8c8d").pack()

        # ── Indicador de confianza ────────────────────────────────
        conf  = _g("conf_tipo")
        color_conf = {"Alta":"#27ae60","Media":"#f39c12","Baja":"#e74c3c"}.get(conf,"#7f8c8d")
        icono_conf = {"Alta":"🟢","Media":"🟡","Baja":"🔴"}.get(conf,"⚪")
        tk.Label(win, text=f"{icono_conf}  Confianza de clasificación: {conf}",
                 font=("Segoe UI",10,"bold"), bg=CF, fg=color_conf).pack(pady=(8,4))

        # ── Secciones de trazabilidad ─────────────────────────────
        fm_sec = tk.Frame(win, bg=CF); fm_sec.pack(fill="both", expand=True, padx=20)

        def _seccion(titulo, datos, bg_color="#16213e"):
            fr = tk.Frame(fm_sec, bg=bg_color, pady=6)
            fr.pack(fill="x", pady=3)
            tk.Label(fr, text=titulo, bg=bg_color, fg=CA,
                     font=("Segoe UI",9,"bold")).pack(anchor="w", padx=10, pady=(2,4))
            for etiq, val in datos:
                fila = tk.Frame(fr, bg=bg_color); fila.pack(fill="x", padx=10, pady=1)
                tk.Label(fila, text=etiq, bg=bg_color, fg="#7f8c8d",
                         font=("Segoe UI",8), width=20, anchor="w").pack(side="left")
                tk.Label(fila, text=str(val)[:90], bg=bg_color, fg="#a8c8f0",
                         font=("Consolas",8), anchor="w").pack(side="left")

        _seccion("📄  CLASIFICACIÓN DOCUMENTAL", [
            ("Tipo detectado:",   _g("tipo")),
            ("Fuente del tipo:",  _g("fuente_tipo")),
            ("Regla aplicada:",   _g("regla_tipo")),
            ("Confianza:",        _g("conf_tipo")),
        ])

        _seccion("📅  FECHA Y HORA", [
            ("Fecha:",            _g("fecha")),
            ("Hora:",             _g("hora")),
            ("Fuente fecha:",     _g("fuente_fecha")),
            ("Regla aplicada:",   _g("regla_fecha")),
        ])

        _seccion("🔢  NÚMERO DE DOCUMENTO", [
            ("N° documento:",     _g("num_doc")),
            ("Regla aplicada:",   _g("regla_num_doc")),
        ])

        _seccion("🔐  INTEGRIDAD SHA-256", [
            ("Hash completo:",    _g("hash_sha256")),
            ("Hash corto:",       hash_corto(_g("hash_sha256","")) or "—"),
        ])

        # Historial de ediciones
        if isinstance(r, dict):
            historial = r.get("historial_ediciones", [])
        else:
            historial = getattr(r, "historial_ediciones", [])

        if historial:
            fr_h = tk.Frame(fm_sec, bg="#16213e", pady=6)
            fr_h.pack(fill="x", pady=3)
            tk.Label(fr_h, text="✎  HISTORIAL DE EDICIONES MANUALES",
                     bg="#16213e", fg="#e67e22",
                     font=("Segoe UI",9,"bold")).pack(anchor="w", padx=10, pady=(2,4))
            for ed in historial:
                fila_h = tk.Frame(fr_h, bg="#16213e"); fila_h.pack(fill="x", padx=10, pady=1)
                ts   = ed.get("timestamp","")
                camp = ed.get("campo","")
                ant  = ed.get("valor_anterior","")
                nvo  = ed.get("valor_nuevo","")
                usr  = ed.get("usuario","")
                tk.Label(fila_h,
                    text=f"{ts}  {usr}  [{camp}]  '{ant}' → '{nvo}'",
                    bg="#16213e", fg="#a8c8f0",
                    font=("Consolas",7.5), anchor="w").pack(side="left")
        else:
            fr_h = tk.Frame(fm_sec, bg="#16213e", pady=6)
            fr_h.pack(fill="x", pady=3)
            tk.Label(fr_h, text="✎  SIN EDICIONES MANUALES — Valores 100% automáticos",
                     bg="#16213e", fg="#27ae60",
                     font=("Segoe UI",9,"bold")).pack(anchor="w", padx=10)

        # ── Cierre ────────────────────────────────────────────────
        self._btn(win, "✖ Cerrar", win.destroy, CR).pack(pady=10, anchor="e", padx=20)

    def _limpiar(self):
        self.tree.delete(*self.tree.get_children())
        self.expediente=None; self.progreso.configure(value=0)
        self.lbl.config(text="Selecciona una carpeta y presiona Analizar.")
        self.lbl_firma.config(text="")
        self.lbl_estado.config(text="",fg="#7f8c8d")
